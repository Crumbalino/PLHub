...
const principles = [
  ...
  },
  {
    ...
    body: [
      'We don\'t make content out of other fans\'s misery. We\'re not '
  ...
  return (
    <div style={{ display: 'flex', gap: '20px',
      ...
      <span style={{ fontSize: '13px', fontWeight: 700,
        color: 'var(--plh-pink)', flexShrink: 0,
        fontFamily: "'JetBrains Mono', monospace",
        paddingTop: '4px' }}>{number}</span>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        <h2 style={{ fontSize: '20px', fontWeight: 700,
          color: 'var(--plh-text-100)', margin: 0,
          fontFamily: "'Sora', sans-serif",
          lineHeight: 1.3 }}>{title}</h2>
        {body.map((para, i) => (
          <p key={i} style={{ fontSize: '16px', lineHeight: 1.75,
            color: 'rgba(250,245,240,0.8)', margin: 0,
            fontWeight: 300, fontFamily: "'Sora', sans-serif" }}>
            {para}
          </p>
        ))}
      </div>
